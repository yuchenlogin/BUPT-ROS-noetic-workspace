...
from ros_vision_node.srv import FaceDetectService, FaceVerifyService
...
def face_gender(gender):
    ...
    if len(face_list) == 0:
        ...
        target_face = face_filter(face_list)
        current_gender = target_face.get('gender').lower()
...
def face_emotion(emotion):
    ...
    if len(face_list) == 0:
        ...
        target_face = face_filter(face_list)
        result = target_face.get('emotion').lower()
        if result == emotion:
            return True
        else:
            return False
    ...
    while True:
        ...
        if len(face_list) > 0:
            ...
            if tmp_result and emotion is not None:
                current_emotion = target_face.get('emotion').lower()
