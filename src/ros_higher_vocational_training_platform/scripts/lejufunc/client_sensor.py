...
def parse_sensor_CF1(sensor_name):
	global sensor_values
	sensor_values = None

	def parse_sensor_msg(msg):
		global sensor_values
		if msg.name == sensor_name:
			sensor_values = msg.values

	sub = rospy.Subscriber("MediumSize/SensorHub/sensor_CF1", ChannelFloat32, parse_sensor_msg)

	while sensor_values is None:
		rospy.sleep(0.1)
...
def fire_sensor():
	...
	res = parse_sensor_CF1("FireStatus")
	result = False
	if res[0]:
		result = True
...
def human_sensor():
	...
	res = parse_sensor_CF1("InfraredStatus")
	result = False
	if res[0]:
		result = True
...
def touch_sensor():
	...
	res = parse_sensor_CF1("TouchStatus")
	result = False
	if res[0]:
		result = True
...
def gas_sensor():
	...
	res = parse_sensor_CF1("GasStatus")
	result = False
	if res[0]:
		result = True
