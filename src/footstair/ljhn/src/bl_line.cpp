...
//--------------------------------------------------------------------------------------
Bline::Bline()
    : m_keyPoints(NULL), m_keyCounts(0), m_parts(NULL), m_partCounts(0)
{
}
...
bool Bline::build(const Real *keyPoints, unsigned int keyCounts)
{
  ...
  for (size_t i = 0; i < m_partCounts; i++)
  {
    ...
    Real az = lp.pt0.z - 2 * lp.pt1.z + lp.pt2.z;
    Real bx = 2 * lp.pt1.x - 2 * lp.pt0.x;
    Real by = 2 * lp.pt1.y - 2 * lp.pt0.y;
    Real bz = 2 * lp.pt1.z - 2 * lp.pt0.z;

    lp.A = 4 * (ax * ax + ay * ay + az * az);
    lp.B = 4 * (ax * bx + ay * by + az * bz);
    lp.C = bx * bx + by * by + bz * bz;
    lp.sqrt_A = sqrt(lp.A);
    lp.sqrt_C = sqrt(lp.C);
    ...
    // std::cout<<" "<<lp.A<<std::endl;
    // std::cout<<" "<<lp.B<<std::endl;
    // std::cout<<" "<<lp.C<<std::endl;
    // std::cout<<" "<<lp.sqrt_A<<std::endl;
    // std::cout<<" "<<lp.sqrt_C<<std::endl;
    // std::cout<<" "<<lp.D<<std::endl;
    // std::cout<<" "<<lp.E<<std::endl;
    // std::cout<<"length: "<<lp.length<<std::endl;
...
void Bline::getPoint(Real t, Point &point, Point &tangent) const
{
  ...
  if (!(t >= 0.0 && t <= 1.0))
  {
    std::cout << "error bazel t: " << t << std::endl;
    if (!(t >= (Real)0.0))
    {
      t = 0.;
    }
