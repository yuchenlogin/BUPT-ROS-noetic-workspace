...
获取全部舵机的扭矩开关状态。
- /MediumSize/BodyHub/DirectMethod/GetRawBulkReadData (bodyhub::SrvGetRawBulkReadData)  
获取舵机或传感器的原始数据。
...
- MediumSize/BodyHub/ScanServo (bodyhub::SrvServoScan)  
  获取舵机状态。服务定义
  ```
  string scanmode
  ---
  ...
  参数形式  
  ```
  request scanmode: "torso": 头部和躯干舵机共22个, id范围(1~22)
                    "fsr": 脚底压感共2个, id范围(111、112)
                    "baseboard": 主板共1个, id(200)
                    "all": "torso" & "fsr" & "baseboard"
  response getData: 以列表形式返回id号，列表长度与id个数相等
  ```
## 调用的服务
无
## 参数
