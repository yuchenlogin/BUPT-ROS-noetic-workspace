...
bool SensorWrite(uint8_t WriteCount, uint8_t *WriteID, uint16_t *WriteAddress,
  ...
  if (bodyhubState != StateEnum::external_control) {//while in external_control status,don't write sensors
...
void SensorbulkReadPoll() {
  if (SensorsList.size()) {
    ...
    if (bodyhubState == StateEnum::external_control){//while in external_control status,don't read sensors
      // ROS_WARN("IN external_control");
      return;
    }
...
bool RegistSensorCallback(bodyhub::SrvInstWrite::Request &req,
  ...
  pthread_mutex_lock(&mtxBs);
  if (bodyhubState != StateEnum::external_control) {
  ...
  pthread_mutex_unlock(&mtxBs);
  ROS_ERROR("bodyhub state is external_control\n");
  res.complete = false;
  return false;
}
...
void sensorTimerThread() {
  ...
  ros::Rate rate_FASTER(20);//20Hz
  ros::Rate rate_SLOWER(5);//5Hz
