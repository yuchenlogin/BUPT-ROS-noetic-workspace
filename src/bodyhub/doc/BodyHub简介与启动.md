...
- 打开`bodyhub/src/BodyHubNode.cpp`文件，在主函数找到`STATEinit`函数，在`STATEinit`函数中找到`const char *portName = "/dev/usb_servo";`代码，修改`"usb_servo"`为上一步记下的值
- 编译ROS工作空间
- 下位机至少连接一个传感器或执行器

#### 运行节点
以下内容假设ttyUSBx号为usb_servo
- 打开终端运行命令`roscore`，打开ros master
- 打开新终端运行命令`sudo chmod 666 /dev/usb_servo`，回车后输入自己的密码确认，修改USB设备权限
- 继续在终端运行命令`setserial /dev/usb_servo low_latency`，设置串口参数
