    ...
    def __image_callback(self, msg):
        ...
        if self.__debug:# 如果在debug模式则显示debug图片和rgb取色窗口
            ...
            if key == ord("s"):
                ...
                cv2.imwrite("/home/bupt/Pictures/infonew/image{}.png".format(self.id),self.__img_origin)
            elif key == ord("w"):
                self.id = time.strftime("%Y%m%d%H%M%S", time.localtime(time.time()))
                print("save negimage{}.png".format(self.id))
                cv2.imwrite("/home/bupt/Pictures/bad_rgb/image{}.png".format(self.id),self.__img_origin)
