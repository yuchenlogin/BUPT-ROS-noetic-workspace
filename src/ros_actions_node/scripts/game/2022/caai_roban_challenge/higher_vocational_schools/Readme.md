<font face="微软雅黑"  size="6"><p align="center">人工智能大赛--高职版说明文档</p></font>
...
```shell
  ...
  $ git pull
```
然后根据更新程序文档`/home/lemon/robot_ros_application/scripts/sys_update/README.md`进行更新。
...
<img title="" src="./image/3-1.jpg" alt="" data-align="center" width="573">
...
高职版比赛第一关是识别出数字1的在四个备选位置中的哪个然后走过去即可。数字识别使用的是opencv里面的模板匹配(有能力的同学也可以换成更高效和适应性更强的特征匹配方法)。Identifier用于数字识别，调用Identifier.get_num_n_pos即获取一次摄像头图像进行模板匹配，找出数字1的位置（左上、左下、右上、右下）。
...
如果无法识别数字位置，可以查看摄像头是否看清四个数字位置。修改 `Task_identify_numbers_n.py` 文件第 33 行，将 `is_debug = False` 改为 `is_debug = True` ，即可显示 opencv 处理后的图像

其中:（按照图像处理的先后顺序排序，依次由先到后）

`origin_img` 是原相机的图像

![原相机图像](./image/%E5%8E%9F%E7%9B%B8%E6%9C%BA%E5%9B%BE%E5%83%8F.png)

`perspective_img` 是透视变换后的图像，如下图如果数字没有看全，需要修改透视变换的图像坐标，在 `Task_identify_numbers_n.py` 文件第 25 行，`POINTS = [[135, 132], [487, 136], [0, 422], [640, 422]]` 依次是左上、右上、左下、右下，可以将鼠标移动到原相机图像上，窗口底部会显示 `(x=鼠标所在图像的水平方向坐标,v=鼠标所在图像的垂直方向坐标)` ， 将鼠标分别移到左上框的左上角、右上框的右上角、左下框的左下角和右下框的右下角，修改对应的 `POINTS` 值

![透视变换](./image/%E9%80%8F%E8%A7%86%E5%8F%98%E6%8D%A2.png)

`thresh` 是二值化图像，如果白色区域太多可以调整二值化的阈值，在`Task_identify_numbers_n.py` 文件第 64 行，原阈值为 150 ，数值越小白色部分越多

![图像二值化](./image/%E5%9B%BE%E5%83%8F%E4%BA%8C%E5%80%BC%E5%8C%96.png)

`match` 是匹配结果图像，在图像中会用黑框框出数字 1 在图中识别的结果，如果未框中，应该考虑修改模板图片的大小。模板图片大小和黑框一致，修改大小的数值在 `Task_identify_numbers.py` 文件第 73 行，`dsize=(75, 75)` 参数依据框和图像中数字大小相应增大或减小，如原代码是缩放成 75*75 像素点大小

![匹配结果](./image/%E8%AF%86%E5%88%AB%E7%BB%93%E6%9E%9C.png)

如果仍不能识别，应考虑更换number_img目录下的模板图片（一般不用更换），模板图片可以从灰度图像获取，截取数字矩形区域作为模板，然后替换掉`higher_vocational_schools/image`里面的对应的图片模板即可。

<img title="" src="./image/1-5.png" alt="" data-align="center">
...
然后以其为中心适当取上下阈值填入HSVColor对应颜色的上下限中即可
...
动作帧文件由pc端Roban软件生成,在该关卡的作用主要是执行推动方块的动作
