...
OLD_CV_FLAG = False
if LooseVersion(current_version) < LooseVersion("3.5.0"):
    print("Old cv version")
    OLD_CV_FLAG = True
...
class Detect_thread(threading.Thread):
    ...
    def color_trace(self,hsv, color):
        ...
        if OLD_CV_FLAG:
            image_copy, contours,__a = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        else:
            contours,__a = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        m = 0
