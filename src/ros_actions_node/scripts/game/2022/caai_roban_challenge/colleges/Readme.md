...
高校比赛第一关是识别出四个数字的在四个备选位置中的位置关系然后依次走过去。数字识别使用的是opencv里面的模板匹配(有能力的同学也可以换成更高效和适应性更强的特征匹配方法)。Identifier用于数字识别，调用Identifier.get_numbers_pos即获取一次摄像头图像进行模板匹配，找出各个数字的位置（左上、左下、右上、右下）。
...
如果无法识别数字位置，可以查看摄像头是否看清四个数字位置。修改 `Task_identify_numbers.py` 文件第 33 行，将 `is_debug = False` 改为 `is_debug = True` ，即可显示 opencv 处理后的图像
...
`perspective_img` 是透视变换后的图像，如果数字没有看全需要修改透视变换的图像坐标，在 `Task_identify_numbers.py` 文件第 25 行，`POINTS = [[162, 60], [481, 60], [40, 380], [620, 380]]` 依次是左上、右上、左下、右下，可以将鼠标移动到原相机图像上，窗口底部会显示 `(x=鼠标所在图像的水平方向坐标,v=鼠标所在图像的垂直方向坐标)` ， 将鼠标分别移到左上框的左上角、右上框的右上角、左下框的左下角和右下框的右下角，修改对应的 `POINTS` 值

![透视变换](./image/%E9%80%8F%E8%A7%86%E5%8F%98%E6%8D%A2.png)

`gray_img` 是灰度图像

![灰度图像](./image/%E7%81%B0%E5%BA%A6%E5%9B%BE%E5%83%8F.png)

`model_img` 是模板图像，分别会显示四张数字图像

![模板图像](./image/%E6%A8%A1%E6%9D%BF%E5%9B%BE%E7%89%87.png)

`model_img_resized` 是匹配结果图像，在图像中会依次按照 1、2、3、4 用黑框在图中框出识别的结果，如果存在未框中的数字，应该考虑修改模板图片的大小。模板图片大小和黑框一致，修改大小的数值在 `Task_identify_numbers.py` 文件第 127 行，`dsize=(256, 256)` 参数依据框和图像中数字大小相应增大或减小，如原代码是缩放成 256*256 像素点大小

![匹配结果](./image/%E6%A8%A1%E6%9D%BF%E5%8C%B9%E9%85%8D%E7%BB%93%E6%9E%9C.png)

如果仍不能识别，应考虑更换number_img目录下的模板图片（一般不用更换），模板图片可以从灰度图像获取，截取数字矩形区域作为模板，然后替换掉`college/image`里面的对应的图片模板即可。
