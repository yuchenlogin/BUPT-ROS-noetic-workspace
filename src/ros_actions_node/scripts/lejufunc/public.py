...
STEP_LEN_MAX = [0.06, 0.02, 8]

PUBLIC_NODE_NAME = "public_node"
class PublicNode(bodact.Action):
    def __init__(self,NODE_NAME=PUBLIC_NODE_NAME,CONTROL_ID=2,init_node=True):
        super(PublicNode, self).__init__(NODE_NAME, CONTROL_ID, init_node)
        ...
        self.err_threshold = [0.10, 0.10, 10]
    ...
    def path_tracking(self, path_point, pos_wait_mode=0,ignore_angle=False,pos_exit_high_acc=True,wait_time=0.4):
        ...
        while not rospy.is_shutdown():
            ...
            if not ignore_angle and v[0]**2+v[1]**2<=(self.err_threshold[0]*2)**2: # use the calibrated direction
                w = step_len[2]
            else: # use the direction along the line between two points
